...
- **The preset, not your command line, is the authority on build flags.** `cmake --preset` passes every `cacheVariables` entry as `-D` on *each* invocation, so it overwrites a standing `CMakeCache.txt`: a flag you force by hand is silently reverted the next time anything re-runs the preset, and `tools/build-fcad.cmd` re-runs it on every build (measured 2026-09-19, where `-DBUILD_FEM=ON` was reverted twice before the value was moved into the preset). What genuinely does not reach an existing tree is a preset edit nobody reconfigures for, and a changed `option()` default -- that is how `build/conda-relwithdebinfo-801`, the tree every test and measurement uses, sat at `BUILD_FEM=OFF` long after the presets said ON, silently running 90 tests short. So change the flag in the preset itself and reconfigure; forcing it on the command line does not stick.
