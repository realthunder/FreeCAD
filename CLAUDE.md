...
- **A preset change does not reach a build tree that already exists.** `cacheVariables` only seed a *fresh* configure, so a standing `CMakeCache.txt` keeps whatever it was first given. That is how `build/conda-relwithdebinfo-801` -- the tree every test and measurement uses -- sat at `BUILD_FEM=OFF` long after the presets said ON, silently running 90 tests short. After editing a preset, either reconfigure the tree with the flags forced on the command line or delete it.
