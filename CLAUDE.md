...
- **Primary: conda-based debug stack** (Qt 6.10.1 + PySide6 6.10.1, gcc 15) -- the only way to get a matched Qt6+PySide6 on Ubuntu 24.04. Env at `.conda/freecad`; wrap every build/run/debug command with `~/works/sw/fcad/.conda/run.sh` (activates env, keeps builds truly Debug). Configure with the `conda-debug-local` user preset -> `build/conda-debug` (Ninja), which links the local debug installs OCCT **7.7.2** in `occt/install/conda-debug` and Coin in `coin/install/conda-debug`. **There is no debug 8.0.1 stack** (verified 2026-08-18): 8.0.1 exists only as the optimized `conda-relwithdebinfo-801` -> `build/conda-relwithdebinfo-801` (OCCT in `occt/install/conda-relwithdebinfo-801`) and `conda-tsan-801`, so anything that must run against the 8.0.1 fork -- or be measured -- builds there, and a debug tree runs the 7.7.2 side of every `OCC_VERSION_HEX` guard. That debug tree was left mid-rebuild and needs a full rebuild before anything out of it is trusted.
