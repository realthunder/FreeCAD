...
- **Primary: the conda RelWithDebInfo stack.** Env at `.conda/freecad`; wrap every build/run/debug command with `~/works/sw/fcad/.conda/run.sh` (activates the env and strips conda's release-tuned `-O2`/`-DNDEBUG`). **The standard build, every test run and every measurement use the `conda-relwithdebinfo-801` preset -> `build/conda-relwithdebinfo-801`** (Ninja), which links OCCT **8.0.1** in `occt/install/conda-relwithdebinfo-801` and Coin in `coin/install/conda-relwithdebinfo`. A debug stack exists alongside it for debugger sessions only -- `conda-debug-local` -> `build/conda-debug-occt801`, against `occt/install/conda-debug-801` and `coin/install/conda-debug`; do not test on it. **7.7.2 is gone from this box** (prefixes and trees deleted 2026-08-28) -- it had already stopped compiling `Mod/Part`, and 7.7.2 is frozen; see `docs/Backport772.md`.
- **FEM is on in both conda presets** (`BUILD_FEM=ON`, `FREECAD_USE_EXTERNAL_SMESH=ON`, `BUILD_FEM_NETGEN=ON`): SMESH comes from our `realthunder` channel package, and everything else it needs -- VTK, MEDFile, HDF5 -- lives in `.conda/freecad` itself. There is no separate dependency prefix and the presets pass no VTK/SMESH/MEDFile/HDF5 paths; that works because the env now tracks upstream's stack (qt6-main/pyside6 6.11.2, boost 1.90, **vtk 9.6.2 pinned** in `conda-meta/pinned` -- do not let it drift). See `docs/DevEnvironment.md`.
- **A preset and a standing cache fight, and which wins depends on what re-runs.** `cmake --preset` passes every `cacheVariables` entry as `-D` on EVERY invocation, so re-running it OVERWRITES a standing cache: a hand-forced `-DFOO=OFF` is back to `ON` the next time the preset runs (measured 2026-09-19). That is silent on any box whose build script reconfigures on each build, where a flag therefore has to live in the preset rather than on the command line. What genuinely does not reach an existing tree is a preset edit nobody reconfigures for, and a changed `option()` default -- a cached value survives both. That second half is how `build/conda-relwithdebinfo-801` -- the tree every test and measurement uses -- sat at `BUILD_FEM=OFF` long after the presets said ON, silently running 90 tests short. So: after editing a preset, reconfigure the tree or delete it; and before forcing a flag with `-D`, ask whether a preset will run again behind you.
- **`cogapp` is a configure-time requirement** (2026-09-12): `pip install cogapp` into `.conda/freecad`, or the configure fails with that line. Some generated sources are no longer committed -- `generate_from_cog()` in `cMake/FreeCadMacros.cmake` writes them into the build tree, the way `generate_from_xml` writes the Python bindings. The 47 files with in-place `[[[cog` blocks still keep their output committed and are regenerated by hand; they migrate to the macro over time. `docs/DevEnvironment.md`, "Generated code: cog".
...
Both suites are green -- Python 2628 OK, C++ 453/453 -- and `ENABLE_DEVELOPER_TESTS=ON`
