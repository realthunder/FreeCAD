...
- `BGFXRenderer.cpp` — the **bgfx** backend (the one to develop), by now a full engine (~14k lines): EVSM/bulb shadows, GTAO, PBR + IBL environment, WBOIT, section caps, outlines, water/glass/cloud/fire volumetrics, bloom, instancing, user shaders, and stateless/stateful GPU particles. `DiligentRenderer.cpp` — a parallel DiligentEngine backend. Enabled with `-DBUILD_BGFX=ON` / `-DBUILD_DILIGENT=ON`.
