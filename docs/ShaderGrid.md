...
wasted-republish chase (`3a2973ae9b`). Supersedes nothing.
...
| `autoLevels` | pick the step from the camera | `GridAuto` | false (new pref, see 10) |
...
- snapping: `ViewProviderGridExtension::getGridSize()` answers from it for
  the desktop camera instead of from `computeGridSize()`, so the sketch
  snaps to exactly the lines drawn.
...
1. Intersects its own view frustum with the grid plane, clamped to
   `halfExtent`, to get the u, v window worth drawing.
2. Measures pixels per unit at the grid (at the window centre in ortho; per
   distance band in perspective, where one level is not enough for a plane
   running to the horizon -- Blender's per-iteration levels).
3. Asks `GridLevels` for the levels, snaps each level's lines to its step
   (the lattice stays fixed in model space while the camera moves) and
   writes the segments into a **transient** instance buffer: the same
   per-instance layout `vs_fc_line` already reads (endpoints, stipple run,
   colour at each end). Level fade, major emphasis, axis colours and the
   perspective distance and grazing fade all go into those per-end colours,
   so the existing line shaders need no change.
4. Submits through the existing thick-line programs (`m_progLine`,
   `m_progLinePat`, `*Clip`), so widths, analytic antialiasing, stipple,
   clip planes and pass/sort behaviour are the ones every other line has.
...
2. The mode-3 capture companion and the backend generator. Gates: pixel
   comparison with mode 0 at a few zooms, and a count test -- 20 zoom steps
   in a sketch edit with the scene server up publish **0** times (the
   session-93 probe reads 13 for one `fitAll` today).
...
4. Migrate `ViewProviderGridExtension`, with a snap-parity test:
...
1. **Automatic levels for Draft.** Fixed stays the default so nothing
   changes under users; should a `gridAuto`-style preference be offered, and
   what does `gridSize` mean with it on (extent, or ignored)?
2. **Snapping from a thin client.** When a browser viewer drives an edit
   (input mirroring), should grid snaps use the viewer's camera -- the grid
   that viewer is showing -- rather than the desktop's? Section 4.1 makes
   either possible, but the host has to know which camera the pointer came
   from.
3. **Perspective.** How many distance bands, and where the horizon cut-off
   sits; Blender's far-clip fade is the obvious default.
4. **Depth.** Keep both grids as ordinary depth-tested lines, as today, or
   offer Blender's "behind geometry" mode?
5. **Upstream.** Both files are upstream's. The Part extension is a small
   class, and the Draft tracker change is contained, but each future
   upstream sync of either file will conflict. Is that acceptable, or should
   the node be proposed upstream once it has proved itself?
