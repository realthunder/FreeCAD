...
| C++ on Windows (`build/win-relwithdebinfo-801`) | **549 of 549 passing** (2026-09-19, after merging RemoteEdit, which adds 52; `BUILD_FEM=ON` adds none, FEM's tests are all Python. 497 on 2026-09-12, including the two new `FileWriterTest` cases; 487 on 2026-09-10, 477 on 2026-09-06/08), 1 disabled -- see "C++ on Windows" |
...
| Python on Windows | **2790 tests, OK** (2026-09-19, after merging RemoteEdit and turning `BUILD_FEM=ON`, which contributes 90; 2600 on 2026-09-15; 2590 on 2026-09-12, the ten new are `FileBlobs.BlobArchiveStoreCases`) -- 0 failures, 0 errors, 80 skipped, 6 expected failures. The 31 skips over the earlier 49 are all sandbox cases idling because `v8-embed` cannot be installed here. **FEM needs `ply` in the env** (`femtools/tokrules.py` imports `ply.lex`) or `test_pyimport_all_FEM_modules` errors. The nine Windows-only failures it carried from 2026-09-07 are fixed; see "Python on Windows" |
