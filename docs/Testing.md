...
| Python on Windows | **2790 tests, 1 failure** (2026-09-20) -- 0 errors, 50 skipped, 6 expected failures. The skips fell 80 -> 50 when the sandbox became functional here, so ~30 `SandboxProgram` / `FeaturePythonChain` cases now genuinely run instead of idling -- and that is what surfaced the one failure, which is therefore newly *visible* rather than new: `SandboxProgramFixtureCases.testProgramsRoutedMatchNative [ProgramFlangeSheet.FCStd]`, "Flange2: the BRep differs". Measured: the BReps differ in exactly one token, `30*sin(45deg)`, by exactly **1.0000 ULP** (MSVC's CRT gives `21.213203435596427`, the wasm guest `...423`) -- the "angles' luck" case `docs/Sandbox.md` 7.17 D3 predicts, now met by a third libm. Note the vertices are bit-identical, so `assertSameGeometry` passes without inspecting the differing value. 2790 OK on 2026-09-19 before the sandbox ran; 2600 on 2026-09-15; 2590 on 2026-09-12, the ten new are `FileBlobs.BlobArchiveStoreCases`. **FEM needs `ply` in the env** (`femtools/tokrules.py` imports `ply.lex`) or `test_pyimport_all_FEM_modules` errors. The nine Windows-only failures it carried from 2026-09-07 are fixed; see "Python on Windows" |
