...
| Suite | Result |
|---|---|
| Python (`FreeCADCmd -t 0`) | **2851 tests, OK** -- 0 failures, 0 errors, 50 skipped, 23 expected failures (2026-09-15, SketcherPort after the phase-1 solver take: +72 from upstream's App-level Sketcher tests, 17 of them marked expected failures until their upstream picks land, +1 document-label test; before that 2778, 6 expected failures, 2026-09-14, after the SecurePython merge, unchanged after merging 7.17 D4; SecurePython alone 2778, RemoteEdit alone 2688). Pass `FCX_PYODIDE` here too: in a fresh `FREECAD_USER_HOME` without it the 28 `SandboxProgram` / `FeaturePythonChain` cases skip ("no sandbox guest in this build") and the run still says OK -- 78 skipped is the tell |
