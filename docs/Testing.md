...
| Suite | Result |
|---|---|
| Python (`FreeCADCmd -t 0`) | **2852 tests, OK** -- 0 failures, 0 errors, 50 skipped, 9 expected failures (2026-09-15, SketcherPort after the internal faces fix in WireJoiner: 3 markers off, +1 fork test; 12 expected failures after phase 2's standalone App fixes: +72 from upstream's App-level Sketcher tests, 6 of them still marked expected failures until their upstream picks land, +1 document-label test; 22 expected failures after the signed-orientation pick; before that 2778, 6 expected failures, 2026-09-14, after the SecurePython merge, unchanged after merging 7.17 D4; SecurePython alone 2778, RemoteEdit alone 2688). Pass `FCX_PYODIDE` here too: in a fresh `FREECAD_USER_HOME` without it the 28 `SandboxProgram` / `FeaturePythonChain` cases skip ("no sandbox guest in this build") and the run still says OK -- 78 skipped is the tell |
