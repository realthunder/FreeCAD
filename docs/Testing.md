...
| Suite | Result |
|---|---|
| Python (`FreeCADCmd -t 0`) | **2913 tests, OK** -- 0 failures, 0 errors, 50 skipped, 6 expected failures (2026-09-25, phase 4 of the transaction log -- branches and trimming: +2 `Document.TransactionBranchCases`, docs/TransactionLog.md sec 26; with `TransactionLog=1` too, 52 skipped; 2911 earlier the same day, re-confirmed after phase 7 of the transaction log -- every commit's values streamed, crash recovery -- with `TransactionLog=1` too, 52 skipped, and with `FC_TXNLOG_CHECK_COPIES` set, docs/TransactionLog.md sec 25.6-25.7; earlier the same day, after the log-on failures were chased, docs/TransactionLog.md sec 24.13: with `TransactionLog=1` also **2911 OK**, 52 skipped -- the two FileBlobs cases that expect content to die skip with the log on; earlier the same day, the blob pack store: +7 `BlobPackStoreCases`, docs/FileBlobsManager.md sec 15.11; with `TransactionLog=1` 6 failures that predate it, docs/TransactionLog.md sec 24.12; 2904 on 2026-09-24, re-confirmed unchanged after undo over the log -- the hot path, cold and selective undo, and restore to a version as a transaction, and view providers through both, and every saved view-provider property logged, docs/TransactionLog.md sec 24.6-24.10 -- and after the fixes of docs/TransactionLog.md sec 23.14, the copy-on-write of sec 23.15 and the blobs through the entity store of sec 23.16; with shape values frozen by default: +2 `ShapeStableBytesCases`, docs/TransactionLog.md sec 23.13; 2902 on `87ef33df5d`, branch Transaction rebased onto LinkVibe: +108 for SketcherPort and the transaction log, and `ShapeStorage.testLocationIsWrittenAsAnAttribute` corrected for the log's fixed float format; 2794 re-measured 2026-09-23 on `bbb926c9f2`, the RemoteEdit merge into LinkVibe: +12 for the cases the two sides added, with skips and expected failures unmoved; 2782 on 2026-09-21 on `a59a8cff4b`: +4 for the Draft and SandboxProgram cases added that day, with skips and expected failures unmoved; 2778 on 2026-09-18 on `932b16a201` and unchanged, which is what clears the Proxy-routing removal: it deletes 15 `__new__` hooks from BIM and Draft product code, and Draft, Arch, Path and Fem are all in this run; 2026-09-14, after the SecurePython merge, unchanged after merging 7.17 D4; SecurePython alone 2778, RemoteEdit alone 2688). Pass `FCX_PYODIDE` here too: in a fresh `FREECAD_USER_HOME` without it the 28 `SandboxProgram` / `FeaturePythonChain` cases skip ("no sandbox guest in this build") and the run still says OK -- 78 skipped is the tell |
| C++ (`ctest`, `ENABLE_DEVELOPER_TESTS=ON`) | **834 of 834 passing** (2026-09-25, phase 4 of the transaction log -- branches and trimming: +9 transaction-log cases, docs/TransactionLog.md sec 26; 834/834 with `TransactionLog=1` and `-j1` too, run with `--timeout 300` since a log gtest hung twice in some 360 runs, sec 26.8; 825 earlier the same day, phase 7 of the transaction log: +3 cases -- the streamed values, the blob store's recovery sweep, a crashed session recovered -- plus a Part case for shapes; 825/825 with `TransactionLog=1` and `-j1` too, docs/TransactionLog.md sec 25.6-25.7; `GuiServeSelectionEcho_tests_run` failed once in a log-on run on a frame count and passed 3 of 3 reruns; 822 earlier the same day: +2 transaction-log cases, the log-on failures chased, docs/TransactionLog.md sec 24.13; with `TransactionLog=1` also 822/822 -- run that with `-j1`, since under `-j6` the test processes race on `user.cfg` and one saves it without the setting, and check `user.cfg` afterwards; 820 earlier the same day, 133 s with `-j6`: +2 recompute-record cases, docs/TransactionLog.md sec 24.11 and 24.12; with `TransactionLog=1` `OmniControl_Tests_run` fails, pack store on or off, sec 24.12; 818 on 2026-09-24, 124 s with `-j6`: +2 transaction-log cases, every saved view-provider property logged and Python values captured on the main thread, docs/TransactionLog.md sec 24.10 -- the GUI audit is `scripts/transaction-log-view-audit.py`; 816, 125 s: +1, view providers through cold undo, sec 24.9 -- its GUI half is `scripts/transaction-log-view-check.py`; 815, 122 s: +1 transaction-log case, selective undo and restore to a version as a transaction, sec 24.7-24.8; 814, 121 s: +3 transaction-log cases, undo over the log, sec 24.6; 811, 122 s: +3 transaction-log cases, blobs through the entity store, docs/TransactionLog.md sec 23.16; 808, 123 s: +1 `ImmutableShapeTest` case, a thawed copy keeps its name, sec 23.15; 807, 122 s: +5 `ImmutableShapeTest` cases for the four freeze regressions of sec 23.14; 802 with shape values frozen by default, 123 s: +4 `ImmutableShapeTest` cases; 798 on `87ef33df5d`, branch Transaction rebased onto LinkVibe, 131 s with `-j6`: the SketcherPort and transaction-log suites, and the 13 guest-image failures that branch carried gone with a rebuilt guest wheel -- which needed the wheel command's `VERBATIM`, `dd85c4a559`, and `-DFCX_IMAGE_VERSION=0.1.0` on a `build/pyodide-guest` configured before the version became three components; 661 re-confirmed 2026-09-23 on `bbb926c9f2`, the RemoteEdit merge into LinkVibe, unchanged in 95.6 s with `-j6`; re-confirmed 2026-09-21 on `a59a8cff4b`, unchanged -- the tests added that day are all Python; 2026-09-18 on `932b16a201`, the Proxy-routing removal: it takes 8 sandbox cases out, and `FormWidgets_Tests_run` needed the exposure fix below to pass in company; 667 of 667 on 2026-09-14, after merging SecurePython sandbox 7.17 D4, which adds 10; 657 with GuiServeClaimChildren; 656 after the first SecurePython merge; SecurePython alone 625, RemoteEdit alone 634), 0 failures, 8 ctest entries disabled, 1 skipped -- 60 of them need the sandbox guest runtime: in a FRESH `FREECAD_USER_HOME` pass `FCX_PYODIDE=$HOME/.local/share/FreeCAD/Pyodide/314.0.6` or they fail with "expression sandbox image is not available". **A merge that brings guest image changes (`src/App/ExpressionImage/`) needs `cmake --build build/pyodide-guest` and `--target fcx_image_wheel` first**: the host build keeps the old wheel, and the first run of this merge failed 16 sandbox cases (`module '_fcx' has no attribute 'surface'`) for that reason alone |
