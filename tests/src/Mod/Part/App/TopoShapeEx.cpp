...
TEST_F(TopoShapeExpansionTest, makECompoundOneShapeReturnsShape)
{
    // Arrange
    ...
    // Act
    topoShape.makECompound(shapes, "C", false);

    // Assert
    EXPECT_EQ(edge.ShapeType(), topoShape.getShape().ShapeType());  // NOT a Compound
}

TEST_F(TopoShapeExpansionTest, makECompoundOneShapeForceReturnsCompound)
{
    // Arrange
    auto edge = BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge();
    TopoShape topoShape {edge};
...
TEST_F(TopoShapeExpansionTest, makECompoundTwoShapesReturnsCompound)
{
    // Arrange
    ...
    // Act
    topoShape.makECompound(shapes);

    // Assert
    EXPECT_EQ(TopAbs_ShapeEnum::TopAbs_COMPOUND, topoShape.getShape().ShapeType());
}

TEST_F(TopoShapeExpansionTest, makECompoundEmptyShapesReturnsEmptyCompound)
{
    // Arrange
    auto edge = BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge();
    TopoShape topoShape {edge};
...
TEST_F(TopoShapeExpansionTest, makECompoundTwoShapesGeneratesMap)
{
    // Arrange
    auto edge1 = BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge();
    auto edge2 = BRepBuilderAPI_MakeEdge(gp_Pnt(1.0, 0.0, 0.0), gp_Pnt(2.0, 0.0, 0.0)).Edge();
...
TEST_F(TopoShapeExpansionTest, makECompoundTwoCubes)
{
    auto [cube1TS, cube2TS] = CreateTwoTopoShapeCubes();
    // Act
    TopoShape topoShape {3L};
    topoShape.makECompound({cube1TS, cube2TS});
...
TEST_F(TopoShapeExpansionTest, resetElementMapTest)
{
    ...
    shapeWithMapAfterEmptyReset.makESHAPE(
        shapeWithMapAfterEmptyReset.getShape(),
        TopoShape::Mapper(),
        {shapeWithMapAfterEmptyReset}
    );

    // A new map. Data::ElementMap is opaque in this fork, so build the map on a
    // scratch shape carrying the same tag and detach it.
    TopoShape mapDonor {3L};
    mapDonor.setElementName(IndexedName("Edge", 2), MappedName("Edge2;:H,E"), nullptr, true);
    auto newElementMapPtr {mapDonor.resetElementMap()};
...
TEST_F(TopoShapeExpansionTest, flushElementMapTest)
{
    ...
    shapeWithMapNotFlushed.makESHAPE(
...
TEST_F(TopoShapeExpansionTest, cacheRelatedElements)
{
    ...
    topoShape.cacheRelatedElements(keyName2, true, names2);
    topoShape.getRelatedElementsCached(keyName, traceType, returnedNames);
    topoShape.getRelatedElementsCached(keyName, true, returnedNames3);
    topoShape.getRelatedElementsCached(keyName2, true, returnedNames2);
    // Assert
...
TEST_F(TopoShapeExpansionTest, makEWiresCombinesAdjacent)
{
    // Arrange
    auto edge1 = BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge();
    auto edge2 = BRepBuilderAPI_MakeEdge(gp_Pnt(1.0, 0.0, 0.0), gp_Pnt(2.0, 0.0, 0.0)).Edge();
    ...
    //    std::vector<TopoShape> shapes {edge1, edge2};
    // Act
    topoShape.makEWires(shapes);
    auto elementMap = topoShape.getElementMap();
    // Assert
    EXPECT_EQ(6, elementMap.size());
}

TEST_F(TopoShapeExpansionTest, makEWiresCombinesWires)
{
    // Arrange
    auto edge1 = BRepBuilderAPI_MakeEdge(gp_Pnt(0.0, 0.0, 0.0), gp_Pnt(1.0, 0.0, 0.0)).Edge();
    auto edge2 = BRepBuilderAPI_MakeEdge(gp_Pnt(1.0, 0.0, 0.0), gp_Pnt(2.0, 0.0, 0.0)).Edge();
    ...
    //    std::vector<TopoShape> shapes {edge1, edge2};
    // Act
    auto& wire1 = (new TopoShape {})->makEWires(shapes);
    auto& wire2 = (new TopoShape {})->makEWires(shapes2);
    auto& topoShape = (new TopoShape {})->makEWires({wire1, wire2});
    auto elements = elementMap((topoShape));
    Base::BoundBox3d bb = topoShape.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEFaceNull)
{
    // Arrange
    ...
    // Act
    TopoShape newFace = topoShape.makEFace(nullptr);
...
TEST_F(TopoShapeExpansionTest, makEFaceSimple)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    double area1 = getArea(topoShape.getShape());
    // Act
    TopoShape newFace = topoShape.makEFace(wire1);
    double area2 = getArea(newFace.getShape());
    double area3 = getArea(topoShape.getShape());
    // Assert
...
TEST_F(TopoShapeExpansionTest, makEFaceParams)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    double area1 = getArea(topoShape.getShape());
    // Act
    TopoShape newFace = topoShape.makEFace(wire1, "Cut", "Part::FaceMakerBullseye", nullptr);
    double area2 = getArea(newFace.getShape());
    double area3 = getArea(topoShape.getShape());
    // Assert
    EXPECT_TRUE(newFace.getShape().IsEqual(topoShape.getShape()));  // topoShape was altered
...
TEST_F(TopoShapeExpansionTest, makEFaceFromFace)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    double area1 = getArea(topoShape.getShape());
    // Act
    TopoShape newFace = topoShape.makEFace(face1, "Cut", "Part::FaceMakerBullseye", nullptr);
    double area2 = getArea(newFace.getShape());
    double area3 = getArea(topoShape.getShape());
    // Assert
    EXPECT_TRUE(newFace.getShape().IsEqual(topoShape.getShape()));  // topoShape was altered
...
TEST_F(TopoShapeExpansionTest, makEFaceOpenWire)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    double area1 = getArea(topoShape.getShape());
    // Act
    TopoShape newFace = topoShape.makEFace(wire1, "Cut", nullptr, nullptr);
    double area2 = getArea(newFace.getShape());
    double area3 = getArea(topoShape.getShape());
    // Assert
    EXPECT_TRUE(newFace.getShape().IsEqual(topoShape.getShape()));  // topoShape was altered
...
TEST_F(TopoShapeExpansionTest, makEFaceClosedWire)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    double area1 = getArea(topoShape.getShape());
    // Act
    TopoShape newFace = topoShape.makEFace(wire2, "Cut", "Part::FaceMakerBullseye", nullptr);
    double area2 = getArea(newFace.getShape());
    double area3 = getArea(topoShape.getShape());
    // Assert
    EXPECT_TRUE(newFace.getShape().IsEqual(topoShape.getShape()));  // topoShape was altered
...
// Possible future makEFace tests:
// Overlapping wire
// Compound of wires
// Compound of faces
...
TEST_F(TopoShapeExpansionTest, getOrderedEdges)
{
    ...
    auto [cube1, cube2] = CreateTwoTopoShapeCubes();
    // Act
    auto subShapes = cube1.getOrderedEdges(false);
    // Assert
...
TEST_F(TopoShapeExpansionTest, getOrderedVertexes)
{
    ...
    auto [cube1, cube2] = CreateTwoTopoShapeCubes();
    // Act
    auto subShapes = cube1.getOrderedVertexes(false);
    // Assert
    EXPECT_EQ(subShapes.size(), 24);
    //    EXPECT_THROW(cube1.getOrderedEdges(), NullShapeException);  // No Map
}
...
TEST_F(TopoShapeExpansionTest, mapSubElementFindShapeByNames)
{
    ...
    topoShape1.makECompound({cube1TS, cube2TS});
...
TEST_F(TopoShapeExpansionTest, mapSubElementFindAncestors)
{
    ...
    topoShape.makECompound({cube1TS, cube2TS});
    topoShape1.makECompound({cube3TS, cube4TS});
    topoShape2.makECompound({cube1TS, cube3TS});
    topoShape3.makECompound({cube2TS, cube4TS});
    topoShape4.makECompound({topoShape, topoShape1});
    topoShape5.makECompound({topoShape2, topoShape3});
    topoShape6.makECompound({topoShape4, topoShape5});
...
TEST_F(TopoShapeExpansionTest, searchSubShapeEverything)
{
    // Arrange
    ...
    // Act
    auto shapes = box1TS.searchSubShape(
        face,
        &names,
    ...
    auto shapes1 = box1TS.searchSubShape(
        edge,
        &names1,
        Data::SearchOption::CheckGeometry,
        tol,
    ...
    auto shapes2 = box1TS.searchSubShape(
        vertex,
        &names2,
        Data::SearchOption::CheckGeometry,
        tol,
...
TEST_F(TopoShapeExpansionTest, searchSubShapeMid)
{
    // Arrange
    auto [box1, box2] = CreateTwoCubes();
    TopoShape box1TS {box1};
...
TEST_F(TopoShapeExpansionTest, searchSubShapeClose)
{
    // Arrange
    auto [box1, box2] = CreateTwoCubes();
    TopoShape box1TS {box1};
...
TEST_F(TopoShapeExpansionTest, makEShellInvalid)
{
    ...
    TopoShape topoShape {1L};
    // Act / Assert
    EXPECT_THROW(topoShape.makEShell(false, nullptr), Base::CADKernelError);
}

TEST_F(TopoShapeExpansionTest, makEShellSingle)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    TopoShape topoShape {face1, 1L};
    // Act
    TopoShape result = topoShape.makEShell(false, nullptr);
    // Assert
...
TEST_F(TopoShapeExpansionTest, makEShellOpen)
{
    // Arrange
    const float Len = 3;
    const float Wid = 2;
    ...
    // Act
    TopoShape result = topoShape.makEShell(true, nullptr);
    // Assert
...
TEST_F(TopoShapeExpansionTest, makEShellClosed)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    ...
    topoShape1.makECompound(shapes, "D");
    // Assert
    TopoShape result = topoShape1.makEShell(false, "SH1");
...
TEST_F(TopoShapeExpansionTest, makEShellIntersecting)
{
    ...
    topoShape1.makECompound(shapes, "D");
    // Assert
    EXPECT_THROW(topoShape1.makEShell(false, nullptr), Base::CADKernelError);
}

TEST_F(TopoShapeExpansionTest, makEShellFromWires)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape topoShape {cube1};
    ...
    topoShape1.makECompound(shapes, "D");
    // Assert
    TopoShape result = topoShape1.makEShellFromWires(shapes);
...
TEST_F(TopoShapeExpansionTest, makEBooleanImpossibleCommon)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    ...
    // Act
    TopoShape& result = topoShape1.makEBoolean(Part::OpCodes::Common, {topoShape1, topoShape2});
...
TEST_F(TopoShapeExpansionTest, makEBooleanCommon)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
...
TEST_F(TopoShapeExpansionTest, makEBooleanCut)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape2 {cube2, 2L};
    // Act
    TopoShape& result = topoShape1.makEBoolean(Part::OpCodes::Cut, {topoShape1, topoShape2});
    auto elements = elementMap(result);
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEBooleanFuse)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape2 {cube2, 2L};
    // Act
    TopoShape& result = topoShape1.makEBoolean(Part::OpCodes::Fuse, {topoShape1, topoShape2});
    auto elements = elementMap(result);
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEDraft)
{  // Draft as in Draft Angle or sloped sides for removing shapes from a mold.
    ...
    const gp_Pln plane {};
    // Act
    TopoShape& result = cube1TS.makEDraft(cube1TS, faces, pullDirection, angle, plane);
    auto elements = elementMap(result);
    // Assert
...
TEST_F(TopoShapeExpansionTest, makEDraftTopoShapes)
{
    // Arrange
    auto [cube1TS, cube2TS] = CreateTwoTopoShapeCubes();
    const gp_Dir pullDirection {0, 0, 1};
    ...
    const gp_Pln plane {};
    // Act
    TopoShape result3 = cube1TS.makEDraft(
        cube1TS.getSubTopoShapes(TopAbs_FACE),
        pullDirection,
        angle,
        plane
    );  // Non Reference call type
    TopoShape result2 = cube1TS.makEDraft(
        cube1TS,
        cube1TS.getSubTopoShapes(TopAbs_FACE),
        pullDirection,
        angle,
    ...
    TopoShape& result = cube1TS.makEDraft(
        cube2TS,
...
TEST_F(TopoShapeExpansionTest, linearizeEdge)
{
    ...
    topoShape1.linearize(false, true);
...
TEST_F(TopoShapeExpansionTest, linearizeFace)
{
    ...
    topoShape2.linearize(true, false);
...
TEST_F(TopoShapeExpansionTest, makERuledSurfaceEdges)
{
    ...
    TopoShape topoShape {1L};
    // Act
    topoShape.makERuledSurface({edge1ts, edge2ts}, 0);  // TODO: orientation as enum?
    auto elements = elementMap(topoShape);
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makERuledSurfaceWires)
{
    ...
    // Act
    auto surface = cube1TS.makERuledSurface(
...
TEST_F(TopoShapeExpansionTest, makELoft)
{
    ...
    // Act
    auto& topoShape = (new TopoShape())->makELoft(shapes, false, false);
    auto& topoShape2 = (new TopoShape())->makELoft(shapes, true, false);
    auto& topoShape3 = (new TopoShape())->makELoft(shapes, false, true);
    auto& topoShape4 = (new TopoShape())->makELoft(shapes, true, true);
    auto& topoShape5
        = (new TopoShape())
              ->makELoft(shapes, false, false, true);
    auto elements = elementMap((topoShape));
...
TEST_F(TopoShapeExpansionTest, makELoftAllowsDistinctProfilesWithSameCenter)  // NOLINT
{
    ...
    //
    // See also test makELoftRejectsCoincidentProfiles, below, to confirm that 5855 is still
    // fixed
    ...
    ASSERT_NO_THROW(loft->makELoft(shapes, false, true));  // NOLINT
...
TEST_F(TopoShapeExpansionTest, makELoftRejectsCoincidentProfiles)  // NOLINT
{
    ...
    //
    // See also makELoftAllowsDistinctProfilesWithSameCenter, above
    ...
    // Act / Assert
    EXPECT_THROW(  // NOLINT
        (new TopoShape())->makELoft(shapes, false, false),
        Base::CADKernelError
    );
}

TEST_F(TopoShapeExpansionTest, makEPipeShell)
{
    // Arrange
    const float Len = 5;
    const float Wid = 5;
    ...
    // Act
    auto& topoShape = (new TopoShape())->makEPipeShell(shapes, false, false);
    auto elements = elementMap((topoShape));
    // Assert that we haven't broken the basic Loft functionality
    EXPECT_EQ(topoShape.countSubElements("Wire"), 4);
...
TEST_F(TopoShapeExpansionTest, makEThickSolid)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape cube1TS {cube1, 1L};
    ...
    // Act
    TopoShape& result = cube1TS.makEThickSolid(cube1TS, shapes, 0.1, 1e-07);
    (void)result;
...
TEST_F(TopoShapeExpansionTest, makEGeneralFuse)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape& result = topoShape1.makEGeneralFuse({topoShape1, topoShape2}, modified);
...
TEST_F(TopoShapeExpansionTest, makEFuse)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape2 {cube2, 2L};
    // Act
    TopoShape& result = topoShape1.makEFuse({topoShape1, topoShape2});  // op, tolerance
    auto elements = elementMap(result);
    // Assert shape is correct
    EXPECT_FLOAT_EQ(getVolume(result.getShape()), 1.75);
    // Assert elementMap is correct
...
TEST_F(TopoShapeExpansionTest, makECut)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape2 {cube2, 2L};
    // Act
    TopoShape& result = topoShape1.makECut(
...
TEST_F(TopoShapeExpansionTest, makEChamfer)
{
    // Arrange
    ...
    // Act
    cube1TS.makEChamfer({cube1TS}, edges, .05, .05);
    auto elements = elementMap(cube1TS);
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEFillet)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape cube1TS {cube1, 1L};
    auto edges = cube1TS.getSubTopoShapes(TopAbs_EDGE);
    // Act
    cube1TS.makEFillet({cube1TS}, edges, .05, .05);
    auto elements = elementMap(cube1TS);
    // Assert shape is correct
    EXPECT_EQ(cube1TS.countSubElements("Wire"), 26);
...
TEST_F(TopoShapeExpansionTest, makESlice)
{
    ...
    // Act
    auto& result = slicer.makESlice(cube1TS, direction, 0.5);
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makESlices)
{
    ...
    Base::Vector3d direction {1.0, 0.0, 0.0};
    // Act
    auto& result = slicer.makESlices(cube1TS, direction, {0.25, 0.5, 0.75});
...
TEST_F(TopoShapeExpansionTest, makEMirror)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape cube1TS {cube1, 1L};
    ...
    // Act
    auto& result = cube1TS.makEMirror(cube1TS, axis);
    auto elements = elementMap(cube1TS);
...
TEST_F(TopoShapeExpansionTest, makETransformWithoutMap)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape1 {cube1, 1L};
    // Act
    TopoShape& result = topoShape1.makETransform(topoShape1, tr);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makETransformWithMap)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    // Act
    TopoShape& result = topoShape1.makEFuse({topoShape1, topoShape2});  // op, tolerance
    topoShape1.makETransform(result, tr);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEGTransformWithoutMap)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    TopoShape topoShape1 {cube1, 1L};
    // Act
    TopoShape& result = topoShape1.makEGTransform(topoShape1, TopoShape::convert(tr));
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(PartTestHelpers::boxesMatch(bb, Base::BoundBox3d(-0.5, -0.5, 0.0, 0.5, 0.5, 1.0)));
...
TEST_F(TopoShapeExpansionTest, makEGTransformWithMap)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    auto tr {gp_Trsf()};
    ...
    // Act
    TopoShape& result = topoShape1.makEFuse({topoShape1, topoShape2});  // op, tolerance
    topoShape1.makEGTransform(result, TopoShape::convert(tr));
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(PartTestHelpers::boxesMatch(bb, Base::BoundBox3d(-0.5, -1.0, 0.0, 1.0, 0.5, 1.0)));
...
TEST_F(TopoShapeExpansionTest, DISABLED_makERevolve)
{
    ...
    // Act
    TopoShape result = subTopoFaces[0].makERevolve(axis, angle);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(
...
TEST_F(TopoShapeExpansionTest, makEPrism)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape topoShape1 {cube1, 1L};
    ...
    subTopoFaces[0].Tag = 2L;
    // Act
    TopoShape& result = topoShape1.makEPrism(subTopoFaces[0], {0.75, 0, 0});
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEPrismUntil)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape cube1TS {cube1, 1L};
    ...
    // Act
    TopoShape result = cube1TS.makEPrismUntil(
        subTopoFaces[4],
        subTopoFaces[4],
        subTopoFaces[5],
        direction,
...
TEST_F(TopoShapeExpansionTest, makEFilledFace)
{
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    TopoShape topoShape1 {cube1, 1L};
    ...
    TopoShape& result = topoShape1.makEFilledFace({topoShape2}, params);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, makEFilledFaceFromLooseEdges)
{
    ...
    auto params = TopoShape::BRepFillingParams();
    result.makEFilledFace(edges, params);
...
TEST_F(TopoShapeExpansionTest, makEFilledFaceExplicitBoundaryRange)
{
...
TEST_F(TopoShapeExpansionTest, DISABLED_makEBSplineFace)
{
    ...
    // Act
    TopoShape& result = topoShape.makEBSplineFace({topoShape2, topoShape3});
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(
...
// Upstream's replaceElementShape and removeElementShape tests are not ported.
// This fork has no element-map-aware replace or remove: replaceShape and
...
TEST_F(TopoShapeExpansionTest, makEEvolve)
{
    ...
    // Act
    TopoShape topoShape {3L};
    auto& result = topoShape.makEEvolve(spine, profile);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(
...
TEST_F(TopoShapeExpansionTest, traceElement)
{
    ...
    // Arrange
    Data::ComplexGeoData::TraceCallback cb =
...
TEST_F(TopoShapeExpansionTest, makEOffset)
{
    // Arrange
    // Arrange
    auto [cube1, cube2] = CreateTwoCubes();
    ...
    TopoShape result {3L};
    //    topoShape1.makEFuse({topoShape2,topoShape2});  // op, tolerance
    result.makEOffset(topoShape1, 0.25, 1e-07);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(
...
TEST_F(TopoShapeExpansionTest, makEOffsetFace)
{
    // Arrange
    const float Len = 3, Wid = 2, Rad = 1;
    auto [face1, wire1, wire2] = CreateFaceWithRoundHole(Len, Wid, Rad);
    ...
    result.makEOffsetFace(result, 0.25, 0);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
...
TEST_F(TopoShapeExpansionTest, DISABLED_makEOffset2D)
{
    ...
    result.makEOffset2D(result, 0.25);
    auto elements = elementMap(result);
    Base::BoundBox3d bb = result.getBoundBox();
    // Assert shape is correct
    EXPECT_TRUE(PartTestHelpers::boxesMatch(bb, Base::BoundBox3d(-0.25, -0.25, 0.0, 3.25, 2.25, 0.0)));
