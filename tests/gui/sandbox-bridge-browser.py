"""A guest in a real browser page reads and writes the served document.

docs/Sandbox.md 7.20, C2. Serves a document headless with a token grant as the
door and FC_BGFX_VIEWER_BUILD at build/wasm, then scripts/console-drive.js
opens web/bridge-test.html?token=...&doc=... in Chrome. The page opens its own
socket to /scene, boots the guest with that socket as its host bridge (JSPI),
runs Python statements against FreeCAD.ActiveDocument and leaves its verdict on
window.fcxBridge; every check of the page is copied here as a PASS/FAIL line,
the bridge figures as a NOTE, and the desktop's document is checked for what
the page wrote.

tests/gui/sandbox-bridge-serve.py drives the same wire without a browser, and
is the registered one. This is not registered, for the reason docs/Testing.md
gives; the environment is the one tests/gui/sandbox-console-browser.py names:
  ...
  scripts/gui-test.sh tests/gui/sandbox-bridge-browser.py /tmp/bridge-web \\
      --timeout 600
"""
...
PAGE = os.path.join(WASM, "web", "bridge-test.html")
DRIVER = os.path.join(REPO, "scripts", "console-drive.js")
DOC = "SandboxBridgeBrowser"
TOKEN = "c2-bridge-token"
RUN_WAIT_S = 420
...
def build():
    try:
        ...
        state["proc"] = subprocess.Popen(
            [node, DRIVER, url, "fcxBridge", str((RUN_WAIT_S - 30) * 1000)],
...
def verify():
    ...
    note(
        "NOTE runtime %s ms, wheels %s ms, bridge %s"
        % (report.get("runtimeMs"), report.get("wheelsMs"), json.dumps(report.get("stats")))
    )
    check("the page's verdict is ok", report.get("ok") is True)

    doc = FreeCAD.getDocument(DOC)
    box = doc.getObject("Box")
    check("the desktop sees the page's write", box.Length.Value == 25, box.Length)
    remote = doc.getObject("Remote")
    check(
        "the desktop sees the page's new object",
        remote is not None and remote.Height.Value == 7,
        remote and remote.Height,
    )
    finish()
