...
GTAO is a stochastic estimator: it marches a few horizon slices per
pixel along directions drawn from a low-discrepancy sequence, so its
output carries per-pixel noise by construction.  Averaging frames is
exactly how that noise is meant to disappear -- but only if successive
frames DIFFER.  Two things stopped them:
  ...
  - the noise itself is pinned to screen position at temporal index 0,
    so even a re-render would have laid down the same pattern.

Both are fixed, and the probe measures the AO buffer itself -- render
debug view mode 3, the AO term alone -- because that is where the
defect lives.  Measurement 2 is the discriminator: on the pre-fix
engine it reads EXACTLY zero by construction, since a buffer that is
never re-rendered is averaged with itself.  Measurement 1 checks the
other side of the same gate -- an idle view WITHOUT the feature must
still hold its cache, which is the same reading of zero, wanted.

The scene is a drilled plate: concave occlusion and nothing else.
"""
import os
import sys
import time
...
def state_prefs():
    ...
    r.SetInt("AOMethod", 1)                # 1 = GTAO (0 = the classic pass)
...
def main():
    ...
    # ---- 1. control: an idle view WITHOUT the feature holds its cache
    view.Render_TemporalAccum = False
    prefs.SetBool("TemporalAccum", False)
    pin(view)
    settle()
    single_p = grab(view, "ao-single")
    pump(SAMPLES + 8)
    single_b = grab(view, "ao-single-b")
    single = load(single_p)
    ctrl = rms(single, load(single_b))
    say("control  : accum off, %d frames apart, AO term rms %.4f (must "
        "be 0 -- the cache still holds when nobody is refining)"
        % (SAMPLES + 8, ctrl))

    # ---- 2. THE discriminator: does the AO buffer move at all? ------
    # Pre-fix this is 0.0000 by construction: the chain is skipped every
    # sample, so the accumulation averages one buffer with itself.  Any
    # nonzero reading means it now re-renders per sample AND lays down
    # different noise when it does.
    ...
    for k in range(4):
        pump(60)
        cur = load(grab(view, "ao-accum-%d" % k))
        steps.append((rms(prev, cur), samples(view)))
        prev = cur
    accum = prev
    d_move = rms(single, accum)
    say("moved    : accumulated AO term vs single-sample rms %.4f "
        "(0 = the chain never re-rendered)" % d_move)
    ...
    say("noise    : high-frequency energy over geometry, single %.4f "
        "-> accumulated %.4f (%.0f%% of it left)"
        % (n_single, n_accum, 100.0 * n_accum / max(n_single, 1e-9)))

    # ---- 4. right, not merely smoother ------------------------------
    # A blur also lowers measurement 3.  What separates converging from
    # blurring is the direction relative to a genuinely supersampled
    # render of the same camera.
    h, w = int(single.shape[0]), int(single.shape[1])
    ref_path = os.path.join(OUT, "ao-ref-%dx.png" % SS)
    pin(view)
    pump(2)
    view.saveImage(ref_path, w * SS, h * SS, "Current")
    big = load(ref_path)[:h * SS, :w * SS]
    # Box-downsample.  No sRGB decode: mode 3 writes the AO term raw,
    # so these values are already linear and averaging them directly is
    # the definition of a supersampled AO pixel.
    ref = big.reshape(h, SS, w, SS, 3).mean(axis=(1, 3))
    Image.fromarray(np.clip(ref, 0, 255).astype(np.uint8)).save(
        os.path.join(OUT, "ao-ref-down.png"))
    d_single = rms(single, ref)
    d_accum = rms(accum, ref)
    say("truth    : AO term vs %dx reference, single %.4f, accumulated "
        "%.4f (lower is closer)" % (SS, d_single, d_accum))
    ...
    ok = (ctrl == 0.0                      # the cache still holds when idle
          and steps[-1][1] >= SAMPLES       # it really accumulated
          and d_move > 0.5                  # THE fix: the chain re-rendered
          and n_accum < n_single            # noise fell
          and d_accum < d_single            # toward truth, not just smoother
          and bd_accum < bd_single          # and the shaded frame improved
