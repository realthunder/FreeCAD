...
#define __FC_PRINT(_instance, _l, _msg_type, _msg, _file, _line)                                        \
    do {                                                                                                \
        if (_instance.isEnabled(_l)) {                                                                  \
            ...
            if (_instance.add_eol)                                                                      \
                _str << std::endl;                                                                      \
            Base::Console().SendMsg<Base::LogStyle::_msg_type>(std::string(), _str.str());  \
...
#define _FC_WARN(_file, _line, _msg)                                                               \
    __FC_PRINT(FC_LOG_INSTANCE, FC_LOGLEVEL_WARN, Warning, _msg, _file, _line)
#define _FC_ERR(_file, _line, _msg)                                                                \
    __FC_PRINT(FC_LOG_INSTANCE, FC_LOGLEVEL_ERR, Error, _msg, _file, _line)
...
Base::ConsoleSingleton::SendMsg(const std::string& notifiername, const std::string &msg)
{
    if (connectionMode == Direct) {
        ...
        postEvent(type, recipient, contenttype, notifiername, msg);
    }
}
