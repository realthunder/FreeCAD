        ...
        for (const char* refName : {"Reference1", "Reference2"}) {
            ...
            if (rigid) {  // Flexible -> Rigid
                if (hasObject(refObj)) {
                    ...
                    // setSubValues is protected here, and setValue is what
                    // recomputes the mapped element names anyway.
                    prop->setValue(this, std::move(newSubs));
                }
            }
            else {  // Rigid -> Flexible
                if (refObj == this) {
                    ...
                    if (child && hasObject(child)) {
                        ...
                        prop->setValue(child, std::move(newSubs));
                    }
                }
            }
        }
...
)
{
    ...
    if (changed) {
        prop2->setValue(prop2->getValue(), std::move(subs1));
    }
}
