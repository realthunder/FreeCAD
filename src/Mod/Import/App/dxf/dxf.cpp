...
void CDxfRead::DoRead(const bool ignore_errors /* = false */)
{
    ...
    try {
        ...
        throw Base::RuntimeError("An unknown, non-standard C++ exception occurred during DXF import.");
    }
}
...
bool CDxfRead::ReadHeaderSection()
{
    ...
    while (get_next_record()) {
        ...
        if (!ReadVariable()) {
            ...
            throw Base::RuntimeError("Failed while reading value for HEADER variable: " + currentVarName);
        }
    }
