...
void FillingPanel::setEditedObject(Surface::Filling* fea)
{
    ...
    if (initFace && subList.size() == 1) {
        QString text =
            QStringLiteral("%1.%2").arg(QString::fromUtf8(initFace->Label.getValue()),
        ...
        try {
            ...
            if (obj && obj->isDerivedFrom<Part::Feature>()) {
                ...
                if (adj_faces.Extent() > 0) {
                    ...
                    ui->comboBoxCont->addItem(QStringLiteral("C0"),
                                              static_cast<int>(GeomAbs_C0));
                    ui->comboBoxCont->addItem(QStringLiteral("G1"),
                                              static_cast<int>(GeomAbs_G1));
                    ui->comboBoxCont->addItem(QStringLiteral("G2"),
