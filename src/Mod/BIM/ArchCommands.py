            ...
            if hasattr(o, "Shape"):
                if is_type_or_link(o, "Window"):
                    ...
                    if hasattr(o, "Hosts") and host not in o.Hosts:
                        o.Hosts += [host]
                        # the host has to re-execute to cut the opening. A plain
                        # Window touches its hosts from _Window.onChanged, but a
                        # Link carries only a shadow Hosts property and reaches
                        # no such hook, so it would never mark the host dirty.
                        # Removal already touches the old hosts, just below.
                        host.touch()
                elif o in outList:
