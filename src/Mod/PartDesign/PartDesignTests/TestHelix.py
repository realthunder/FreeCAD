...
class TestHelix(unittest.TestCase):
    ...
    def testGiantHelixSubtractive(self):
        ...
        for iexponent in range(mine, maxe):
            ...
            helix.Angle = 0
            helix.Mode = 0
            # OCCT 8.0.1: with the default 0.1 the cut at exponent -1 lands
            # 9e-6 mm^3 off (8.9e-3 in the scaled units checked below)
            helix.Tolerance = 0.12
            self.Doc.recompute()
    ...
    def testLegacySweep(self):
        """A helix from before version 3 keeps its MakePipeShell sweep"""
        self.testCone()
        helix = self.Doc.getObject("AdditiveHelix")
        self.assertEqual(helix._ProfileBasedVersion, 3)
        self.assertNotIn("Hidden", helix.getPropertyStatus("Tolerance"))
        helix._ProfileBasedVersion = 2
        self.Doc.recompute()
        self.assertIn("Hidden", helix.getPropertyStatus("Tolerance"))
