...
void Mirroring::onSelectionChanged(const Gui::SelectionChanges &msg)
{
    if (ui->selectButton->isChecked()) {
        if (msg.Type == Gui::SelectionChanges::AddSelection) {
            ...
            ui->referenceLineEdit->setText(QString::fromUtf8(refStr.str().c_str()));
...
bool Mirroring::accept()
{
    ...
    QRegularExpression rx(QStringLiteral(R"( \(Mirror #\d+\)$)"));
    ...
    for (auto item : items) {
        ...
        label = Base::Tools::escapeEncodeString(item->text(0));
