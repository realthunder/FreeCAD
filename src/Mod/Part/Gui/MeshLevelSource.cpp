...
/// Everything a level build needs, owned by the registered closure: a
/// refcounted handle of the exact shape the display tessellation
/// meshed, and a small cache of re-meshed structure copies so the face
/// and edge chunks of one object share one coarse triangulation — that
/// sharing is the artifact fix (the edge polylines index the very
/// nodes the coarse faces are drawn from).
struct LevelSourceState {
    TopoDS_Shape shape;
    bool normalsFromUV = false;
    /// Whole-shape bounds, taken at registration on the GUI thread —
    /// the same box for the face and the edge role, which is what
    /// makes their deviations (and so their triangulations) identical.
...
/// The re-meshed copy for \a level, built on first use. A *structure*
/// copy: fresh TShapes so the coarse triangulation never touches the
/// live shape the GUI reads, stored meshes carried so purely
/// triangulated faces (no surface to re-mesh) keep their geometry.
TopoDS_Shape meshedCopyAt(LevelSourceState &st, uint32_t level,
                          double deflection, double angle)
{
    std::lock_guard<std::mutex> guard(st.mutex);
    auto it = st.meshed.find(level);
    if (it != st.meshed.end())
        return it->second;

    BRepBuilderAPI_Copy copier(st.shape, /*copyGeom*/ Standard_False,
    ...
    // A handful of levels at most; no eviction worth having.
    if (st.meshed.size() < 8)
        st.meshed.emplace(level, copy);
    return copy;
}
...
bool buildFaceLevel(LevelSourceState &st,
...
bool buildEdgeLevel(LevelSourceState &st,
                    const Render::ParsedMeshChunk &src,
                    const TopoDS_Shape &copy, double deflection,
                    double angle, std::vector<uint8_t> &out)
{
...
/// The registered generator body, on the server's level worker thread.
bool buildLevel(const LevelSourceStatePtr &st, uint32_t level,
                ...
                std::vector<uint8_t> &out)
{
    ...
    try {
        ...
        bool ok = src.numTriangleIndices > 0
            ? buildFaceLevel(*st, src, copy, deflection, angle, chunk)
            : src.numLineIndices > 0
                ? buildEdgeLevel(*st, src, copy, deflection, angle, chunk)
                : false;
        if (debugOn() && !ok)
                                      ...
                                      SoNode *lineTag)
{
    ...
    try {
        ...
        if (bounds.IsVoid())
            return;
        bounds.Get(st->bbMin[0], st->bbMin[1], st->bbMin[2], st->bbMax[0],
                   st->bbMax[1], st->bbMax[2]);
        double dx = st->bbMax[0] - st->bbMin[0];
        double dy = st->bbMax[1] - st->bbMin[1];
        double dz = st->bbMax[2] - st->bbMin[2];
        st->diagonal = std::sqrt(dx * dx + dy * dy + dz * dz);
    }
    ...
    auto gen = [st](uint32_t level, const void *chunk, size_t size,
                    std::vector<uint8_t> &out) {
        return buildLevel(st, level, chunk, size, out);
    };
