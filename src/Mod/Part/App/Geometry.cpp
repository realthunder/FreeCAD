...
{
    // Clone, so the Python object carries the geometry extensions too.
...
GeomBSplineCurve*
GeomCurve::toBSpline(const Handle(Geom_Curve) & c, double first, double last, double tol)
{
...
GeomBSplineSurface::GeomBSplineSurface(const Adaptor3d_Surface& surface)
{
    ...
    if (u1 != surface.FirstUParameter() || v1 != surface.FirstVParameter()
        || u2 != surface.LastUParameter() || v2 != surface.LastVParameter()) {
        Trim(surface.FirstUParameter(),
...
void GeomBSplineSurface::Trim(double u1, double u2, double v1, double v2)
{
    try {
        ...
        FC_THROWM(Base::CADKernelError,
                  "Failed to split BSpline surface: " << e.GetMessageString());
    }
}
