    ...
    if (objs && PyObject_TypeCheck(obj, &App::DocumentObjectPy::Type)) {
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            if (objs && PyObject_TypeCheck((*it).ptr(), &App::DocumentObjectPy::Type)) {
                Py::Object pyItem(*it);
                App::DocumentObject *docObj = static_cast<App::DocumentObjectPy*>(pyItem.ptr())->getDocumentObjectPtr();
                objs->emplace_back(docObj);
                shapes.push_back(Part::Feature::getTopoShape(docObj));
                continue;
            }
...
PartExport int initOCCTExtension()
{
    ...
    if (extVersion == 0) {
        extVersion = -1;
#ifdef FC_OS_WIN32
        // LoadLibraryA, not LoadLibrary: the build defines UNICODE, so the
        // unsuffixed macro resolves to LoadLibraryW and rejects this narrow
        // literal. GetProcAddress below is narrow-only in either case.
        HMODULE hModule = LoadLibraryA("TKBRep.dll");
        if (hModule) {
...
public:
    Module() : Py::ExtensionModule<Module>("Part")
    {
        ...
        add_varargs_method("showShapeOCCT", &Module::showShapeOCCT,
            "showShapeOCCT(enable: Boolean)\n"
            "Global enable/disable showing inermediate shape generated by OCCT modeling algorithm.\n\n"
            "showShapeOCCT(key : String, enable = True : Boolean)\n"
            "Fine control of enabling/disabling showing intermediate shapes generated by OCCT modeling algorithm.\n\n"
                ...
                if (PySequence_Check(pcPyShapeOrList)){
                    ...
                    for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                        ...
                        if (PyObject_TypeCheck(item.ptr(), &(Part::TopoShapePy::Type))) {
                            const TopoDS_Shape& sh = static_cast<Part::TopoShapePy*>(item.ptr())->getTopoShapePtr()->getShape();
                            fm->addShape(sh);
                        } else {
    ...
    Py::Object getSortedClusters(const Py::Tuple& args)
    {
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            if (PyObject_TypeCheck(item.ptr(), &(Part::TopoShapePy::Type))) {
                ...
                throw Py::TypeError("item.ptr() is not a shape");
            }
        }
