...
private:
    // upstream 057d51f846 (the probe of its preselection tests), on the
    // fork's own hover pick
    Py::Object getActiveSketchPreselection(const Py::Tuple& args)
    {
        ...
        auto* vp = dynamic_cast<SketcherGui::ViewProviderSketch*>(editDoc->getInEdit());
        if (!vp) {
            return Py::None();
        }
        ...
        if (!vp->getPreselectionAtViewportPos(
        ...
        auto* sketchObject = vp->getObject();
        if (!sketchObject) {
            return Py::None();
        }
