...
public:
    ...
    /** Cycle the segment and transition mode of the segment being drawn.
     *
     * Line or arc, meeting the previous curve freely, tangentially or
     * perpendicularly. Upstream reaches this with the M key alone; here the
     * tool bar reaches it too (toggle() below), which is why geom may be
     * null -- the M key requires a previous curve, a button press need not.
     */
    void cycleSegmentAndTransitionMode()
    {
        ...
        // Null when there is no previous curve. The M key cannot reach that
        // case (it requires one); a tool bar press can.
        ...
        if (SegmentMode == SEGMENT_MODE_Line) {
            switch (TransitionMode) {
                case TRANSITION_MODE_Free:
                    if (geom && geom->is<Part::GeomArcOfCircle>()) {  // 3rd mode
                ...
                case TRANSITION_MODE_Tangent:
                    if (geom && geom->is<Part::GeomArcOfCircle>()) {  // 1st mode
                        TransitionMode = TRANSITION_MODE_Perpendicular_L;
                    }
                    else {  // 3rd mode
                        SegmentMode = SEGMENT_MODE_Arc;
    ...
    /** Pressing the polyline button again cycles the mode instead of
     * restarting the tool: SketcherGui::ActivateHandler offers the press to
     * the running handler first. The same cycle M runs, reached from the
     * tool bar -- which is the route a client without key bindings has, M
     * being a raw Coin key constant and not a command with a shortcut.
     */
