...
void DrawSketchHandlerDragAutoConstraint::initDragging(const std::vector<GeoElementId>& dragged)
{
    ...
    // The fork reaches the view through ViewerContext, so that a client's
    // mirror -- which has no widget -- simply keeps its cursor.
    if (Gui::ViewerContext* viewer = getViewer()) {
