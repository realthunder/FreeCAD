...
void ConstraintView::swapNamedOfSelectedItems()
{
    ...
    std::string escapedstr1 = Base::Tools::escapeEncodeString(
    ...
    : QListWidget(parent)
{
    ...
    // Text constraints are not listed, so their filter would do nothing
...
bool TaskSketcherConstraints::isConstraintFiltered(QListWidgetItem* item)
{
    ...
    // A Text constraint is not listed: it is edited through its geometry, and its filter
    // entry is hidden for the same reason.
