...
public:
    ...
    virtual bool allowExternalDocument() const
    {
        return false;
    }
    ...
    /** Called when the corresponding tool button is pressed while this
     * handler is active; 'next' is the new handler of the pressed button.
     * Return true to discard 'next' and reuse the current handler.
     */
...
public:
    explicit SketcherSelectionFilterGate(App::DocumentObject* obj);
