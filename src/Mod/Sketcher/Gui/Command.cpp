...
void CmdSketcherMirrorSketch::activated(int iMsg)
{
    ...
    for (std::vector<Gui::SelectionObject>::const_iterator it = selection.begin();
        ...
        std::vector<const Part::Geometry*> tempgeo = tempsketch->getInternalGeometry();
        ...
        std::vector<const Part::Geometry*> mirrorgeo(tempgeo.begin() + (addedGeometries + 1),
                                               tempgeo.end());
...
public:
    CmdSketcherGrid();
    ~CmdSketcherGrid() override = default;
