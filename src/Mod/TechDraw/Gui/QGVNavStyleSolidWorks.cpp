...
{
//    Base::Console().Message("QGVNSSolidWorks::handleMouseMoveEvent() - buttons: %d modifiers: %X\n",
...
//                            QGuiApplication::keyboardModifiers().testFlag(Qt::ShiftModifier));
    ...
    if ((QGuiApplication::mouseButtons() & Qt::MiddleButton) &&
        ...
        //pan mode 1 - CNTL + MMB
        //pan mode 2 (MMB alone) would catch this too, but nav style OCC keeps the
        //same explicit block and it documents what the binding is meant to be
        if (panningActive) {
            pan(event->pos());
        } else {
            startPan(event->pos());
