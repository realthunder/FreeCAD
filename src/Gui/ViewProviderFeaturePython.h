...
/** The fork's older spellings of the four names above
 *
 * Upstream calls these ViewProviderFeaturePython*; this fork called them
 * ViewProviderPythonFeature* (docs/UpstreamCoreSync.md section 3, step 2c).
 * The aliases hold the existing call sites -- 52 files name the T form
 * alone -- while new and ported code uses the upstream spelling. They live
 * in this header rather than in the forwarding one at the old path,
 * because a file can reach these declarations through a transitive
 * include and would not see aliases parked in the shim.
 *
 * Note ViewProviderPythonGeometry: upstream's spelling swaps the two words
 * the other way round, to ViewProviderGeometryPython.
 *
 * WARNING: the T form is a template, so its old name has to be an alias
 * template, and an alias template is not a template-name everywhere the
 * real one is. Deriving from it and naming a specialization are fine, but
 * it **cannot be explicitly instantiated** and cannot be
 * forward-declared. The 30 `template class <MOD>Export
 * ViewProviderFeaturePythonT<X>;` lines across 25 files therefore name the
 * real template, which is right anyway -- an explicit instantiation is a
 * definition, not a use.
 */
using ViewProviderPythonFeatureImp = ViewProviderFeaturePythonImp;
template<class ViewProviderT>
using ViewProviderPythonFeatureT = ViewProviderFeaturePythonT<ViewProviderT>;
using ViewProviderPythonFeature = ViewProviderFeaturePython;
using ViewProviderPythonGeometry = ViewProviderGeometryPython;
