...
void ViewProviderSuppressibleExtension::extensionMergeOverlayIcons(QIcon& icon) const
{
    if (isSetSuppressedIcon) {
        static QPixmap px(Gui::BitmapFactory().pixmapFromSvg("feature_suppressed", QSize(16, 16)));

        icon = Gui::BitmapFactoryInst::mergePixmap(icon, px, Gui::BitmapFactoryInst::TopLeft);
    }
}
