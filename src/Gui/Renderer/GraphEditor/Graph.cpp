...
// ImGui::SetWindowFontScale, retired with the obsolete API in 1.92:
// its body, on the window's own scale field.
void setWindowFontScale(float scale)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->FontWindowScale == scale)
        return;
    window->FontWindowScale = scale;
    ImGui::UpdateCurrentFontSize(0.0f);
}

// Whether a key on the canvas is the graph's to act on. The MaterialX
// editor asked IsWindowFocused(RootWindow), which is an identity test
// against ImGui's nav window; a click on the canvas focuses the node
// editor's CHILD window, so that never held here. Any window of the
// root's own hierarchy counts, popups (the node search) do not, and
// neither does a live widget (a text field taking Backspace).
    ...
    _previewSize(128.0f)
{
    ...
    // An empty document until setDocument: the node list comes from
    // the shared data library, the document only refers to it.
    _graphDoc = mx::createDocument();
    if (mx::ConstDocumentPtr library = Render::MaterialX::dataLibrary())
    {
        _graphDoc->setDataLibrary(library);
        createNodeUIList(library);
    }
    initializeGraph();
}
...
std::string Graph::documentText() const
{
    return mx::writeToXmlString(_graphDoc);
}
...
void Graph::setName(const std::string& name)
{
    ...
    {
        _parentStates.front().name = name;
    }
}

bool Graph::takeChange(std::string& xml)
{
    if (!_commitReady)
    {
        return false;
...
void Graph::showPropertyEditorValue(UiNodePtr node, mx::InputPtr input, const UIProperties& uiProperties)
{
    ...
    else if (input->getType() == "filename")
    {
        ...
        if (val && val->isA<std::string>())
        {
            ...
            // Thumbnail from the host, when it has one for the name.
            if (!temp.empty() && _previewSize > 0)
            {
                int width = 0, height = 0;
                ImTextureID textureId = _host->thumbnail(temp, width, height);
                if (textureId != ImTextureID_Invalid && width > 0 && height > 0)
                {
                    ...
                    ImGui::Image(textureId, ImVec2(previewSize, previewSize / aspect));
                }
            }
...
void Graph::createNodeUIList(mx::ConstDocumentPtr doc)
{
    _nodesToAdd.clear();
...
void Graph::buildUiBaseGraph(mx::DocumentPtr doc)
{
    ...
    // With a data library attached, the document's accessors answer
    // the library's children too (that is how nodedefs resolve); the
    // graph shows only what belongs to this document. The MaterialX
    // editor keyed this on source URIs; the owning document is exact.
    mx::ElementPredicate includeElement = [this](mx::ConstElementPtr elem)
    {
        return elem->getDocument().get() == _graphDoc.get();
    };
    ...
    for (UiNodePtr node : _state.nodes)
    {
        ...
        else
        {
            ...
            if (node->getNode() != nullptr)
            {
                ed::BeginNode(node->getId());
                ImGui::PushID(node->getId());
                setWindowFontScale(1.2f * _fontScale);
...
bool Graph::checkCanAddLink(ed::PinId startPinId, ed::PinId endPinId)
{
    ...
    {
        mx::NodeDefPtr nodeDef = _state.nodes[upNode]->getNode()->getNodeDef();
        if (!nodeDef || !_host->hasImplementation(*nodeDef))
        {
...
void Graph::graphButtons()
{
    ...
    // No File menu: the document is the program's property, and undo
    // is the application's (docs/ShaderGraphEditor.md sec 4.4).
    ...
    // The preview is square; the host draws it or draws nothing.
    ImVec2 screenSize = ImVec2(paneWidth, paneWidth);
                ...
                if (haveTable)
                {
                    ...
                    for (UiPinPtr input : _currUiNode->getInputPins())
                    {
                        if (_currUiNode->getShowAllInputs() || (input->getConnected() || _currUiNode->getNode()->getInput(input->getName())))
                        {
                            ...
                            getUIProperties(input->getInput(), mx::EMPTY_STRING, uiProperties);
            ...
            if (count)
            {
                ...
                if (haveTable)
                {
                    ...
                    for (size_t i = 0; i < inputs.size(); i++)
                    {
                        ...
                        UIProperties uiProperties;
                        getUIProperties(mxinput, mx::EMPTY_STRING, uiProperties);
...
{
    bool open_AddPopup = (canvasHasKeyboard() && ImGui::IsKeyReleased(ImGuiKey_Tab)) ||
    ...
    if (ImGui::BeginPopup("add node"))
    {
        ...
        for (auto node : _nodesToAdd)
        {
            ...
            if (subs.size() > 0)
            {
                ...
                {
                    if (ImGui::MenuItem(getUserNodeDefName(nodeName).c_str()) || (ImGui::IsItemFocused() && ImGui::IsKeyPressed(ImGuiKey_Enter)))
                    {
            ...
            else
            {
                ...
                if (ImGui::BeginMenu(node.getGroup().c_str()))
                {
                    ...
                    {
                        if (ImGui::MenuItem(getUserNodeDefName(name).c_str()) || (ImGui::IsItemFocused() && ImGui::IsKeyPressed(ImGuiKey_Enter)))
                        {
        ...
        if (std::string(input).size() > 0)
        {
            for (UiNodePtr node : _state.nodes)
            {
                if (node->getName().find(std::string(input)) != std::string::npos)
                {
                    ...
                    if (ImGui::MenuItem(node->getName().c_str()) || (ImGui::IsItemFocused() && ImGui::IsKeyPressed(ImGuiKey_Enter)))
                    {
...
void Graph::handleRenderViewInputs()
{
    ...
    {
        _host->previewMouse(mousePos.x, mousePos.y, -1, false);
    }
    for (int button = 0; button < 2; ++button)
    {
        if (ImGui::IsMouseClicked(button))
        {
            _host->previewMouse(mousePos.x, mousePos.y, button, true);
        }
        else if (ImGui::IsMouseReleased(button))
        {
            _host->previewMouse(mousePos.x, mousePos.y, button, false);
        }
    }
    if (scrollAmt != 0)
    {
        _host->previewScroll(scrollAmt);
    }
}
...
void Graph::drawGraph(ImVec2 mousePos)
{
    ...
    ed::Begin("My Editor");
    {
        ...
        else if (_layoutPending || _autoLayout)
        {
            ...
            if (!_autoLayout)
            {
                // The layout a document without positions gets on
                // opening is its baseline, not an edit; the user's
                // Auto Layout is one.
                savePosition();
                _committedText = documentText();
            }
        ...
        if (canvasHasKeyboard())
        {
            ...
            else if (ImGui::IsKeyReleased(ImGuiKey_U) && (!ImGui::IsPopupOpen("add node")) && (!ImGui::IsPopupOpen("search")))
            {
                upNodeGraph();
            }
        }
    ...
    shaderPopup();
    bool updateSettled = false;
    if (_updatePending && ImGui::GetFrameCount() >= (_frameCount + 2))
    {
        _updatePending = false;
        updateSettled = true;
        updateMaterials();
    ...
    // A completed gesture -- a button or key let go, or a topology
    // change settled -- with nothing held and nothing being typed
    // into: the document, positions included, is compared with what
    // was last committed, so a click that changes nothing, a value
    // drag or a rename each cost one commit, on release.
    bool gestureEnd = updateSettled || ImGui::IsMouseReleased(0) || ImGui::IsMouseReleased(1);
    for (int key = ImGuiKey_NamedKey_BEGIN; !gestureEnd && key < ImGuiKey_NamedKey_END; ++key)
    {
        gestureEnd = ImGui::IsKeyReleased((ImGuiKey) key);
    }
    if (gestureEnd && !_updatePending && !ImGui::IsAnyItemActive() && !ImGui::IsAnyMouseDown())
    {
        savePosition();
        std::string text = documentText();
        if (text != _committedText)
        {
            _committedText = std::move(text);
            _commitReady = true;
        }
    }
