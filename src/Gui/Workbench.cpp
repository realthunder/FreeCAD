...
void StdWorkbench::setupContextMenu(const char* recipient, MenuItem* item) const
{
    ...
    if (strcmp(recipient,"View") == 0)
    {
        ...
        *item << "Std_ViewFitAll" << "Std_ViewSelection" << "Std_DrawStyle"
...
MenuItem* StdWorkbench::setupMenuBar() const
{
    ...
    *file << "Std_New" << "Std_Open" << "Std_OpenDirectory" << "Std_RecentFiles" << "Separator" << "Std_CloseActiveWindow"
    ...
    *view << "Std_ViewCreate" << "Std_ViewSplitRight" << "Std_ViewSplitDown" << "Std_ViewSplitClose" << "Std_ViewSplitMaximize" << "Std_ViewCellShowObject" <<"Std_OrthographicCamera" << "Std_PerspectiveCamera" << "Std_MainFullscreen" << "Separator"
    ...
    if (DockWindowManager::instance()->isOverlayActivated() &&
        Application::Instance->commandManager().getCommandByName("Std_DockOverlay"))
...
ToolBarItem* StdWorkbench::setupToolBars() const
{
    ...
    *view << "Std_ViewFitAll" << "Std_ViewSelection" << "Std_DrawStyle" << "Std_SelOptions" << "Std_TreeViewActions"
          << "Separator" << "Std_SelBack" << "Std_SelForward" << "Std_SelUp"
...
DockWindowItems* StdWorkbench::setupDockWindows() const
{
    ...
    // The combo view keeps the left side only while it still carries the model
    // tree; reduced to the task panel it goes opposite the tree, on the right.
    root->addDockWidget("Std_ComboView", MainWindow::comboViewDockArea(), true, true);
