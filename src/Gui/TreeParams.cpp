...
public:
    ...
    TreeParamsP() {
        ...
        SyncSelection = this->handle->GetBool("SyncSelection", true);
        funcs["SyncSelection"] = &TreeParamsP::updateSyncSelection;
        CheckBoxesSelection = this->handle->GetBool("CheckBoxesSelection", false);
        funcs["CheckBoxesSelection"] = &TreeParamsP::updateCheckBoxesSelection;
        SyncView = this->handle->GetBool("SyncView", true);
        funcs["SyncView"] = &TreeParamsP::updateSyncView;
        PreSelection = this->handle->GetBool("PreSelection", true);
        funcs["PreSelection"] = &TreeParamsP::updatePreSelection;
        SyncPlacement = this->handle->GetBool("SyncPlacement", false);
        funcs["SyncPlacement"] = &TreeParamsP::updateSyncPlacement;
        RecordSelection = this->handle->GetBool("RecordSelection", true);
        funcs["RecordSelection"] = &TreeParamsP::updateRecordSelection;
        DocumentMode = this->handle->GetInt("DocumentMode", 2);
        funcs["DocumentMode"] = &TreeParamsP::updateDocumentMode;
        StatusTimeout = this->handle->GetInt("StatusTimeout", 100);
        funcs["StatusTimeout"] = &TreeParamsP::updateStatusTimeout;
        SelectionTimeout = this->handle->GetInt("SelectionTimeout", 100);
        funcs["SelectionTimeout"] = &TreeParamsP::updateSelectionTimeout;
        PreSelectionTimeout = this->handle->GetInt("PreSelectionTimeout", 500);
        funcs["PreSelectionTimeout"] = &TreeParamsP::updatePreSelectionTimeout;
        PreSelectionDelay = this->handle->GetInt("PreSelectionDelay", 700);
        funcs["PreSelectionDelay"] = &TreeParamsP::updatePreSelectionDelay;
        PreSelectionMinDelay = this->handle->GetInt("PreSelectionMinDelay", 200);
        funcs["PreSelectionMinDelay"] = &TreeParamsP::updatePreSelectionMinDelay;
        RecomputeOnDrop = this->handle->GetBool("RecomputeOnDrop", true);
        funcs["RecomputeOnDrop"] = &TreeParamsP::updateRecomputeOnDrop;
        KeepRootOrder = this->handle->GetBool("KeepRootOrder", true);
        funcs["KeepRootOrder"] = &TreeParamsP::updateKeepRootOrder;
        TreeActiveAutoExpand = this->handle->GetBool("TreeActiveAutoExpand", true);
        funcs["TreeActiveAutoExpand"] = &TreeParamsP::updateTreeActiveAutoExpand;
        TreeActiveColor = this->handle->GetUnsigned("TreeActiveColor", 3873898495);
        funcs["TreeActiveColor"] = &TreeParamsP::updateTreeActiveColor;
        TreeEditColor = this->handle->GetUnsigned("TreeEditColor", 2459042047);
        funcs["TreeEditColor"] = &TreeParamsP::updateTreeEditColor;
        SelectingGroupColor = this->handle->GetUnsigned("SelectingGroupColor", 1082163711);
        funcs["SelectingGroupColor"] = &TreeParamsP::updateSelectingGroupColor;
        TreeActiveBold = this->handle->GetBool("TreeActiveBold", true);
        funcs["TreeActiveBold"] = &TreeParamsP::updateTreeActiveBold;
        TreeActiveItalic = this->handle->GetBool("TreeActiveItalic", false);
        funcs["TreeActiveItalic"] = &TreeParamsP::updateTreeActiveItalic;
        TreeActiveUnderlined = this->handle->GetBool("TreeActiveUnderlined", false);
        funcs["TreeActiveUnderlined"] = &TreeParamsP::updateTreeActiveUnderlined;
        TreeActiveOverlined = this->handle->GetBool("TreeActiveOverlined", false);
        funcs["TreeActiveOverlined"] = &TreeParamsP::updateTreeActiveOverlined;
        Indentation = this->handle->GetInt("Indentation", 0);
        funcs["Indentation"] = &TreeParamsP::updateIndentation;
        LabelExpression = this->handle->GetBool("LabelExpression", false);
        funcs["LabelExpression"] = &TreeParamsP::updateLabelExpression;
        IconSize = this->handle->GetInt("IconSize", 0);
        funcs["IconSize"] = &TreeParamsP::updateIconSize;
        FontSize = this->handle->GetInt("FontSize", 0);
        funcs["FontSize"] = &TreeParamsP::updateFontSize;
        ItemSpacing = this->handle->GetInt("ItemSpacing", 0);
        funcs["ItemSpacing"] = &TreeParamsP::updateItemSpacing;
        ItemBackground = this->handle->GetUnsigned("ItemBackground", 0x00000000);
        funcs["ItemBackground"] = &TreeParamsP::updateItemBackground;
        ItemBackgroundPadding = this->handle->GetInt("ItemBackgroundPadding", 0);
        funcs["ItemBackgroundPadding"] = &TreeParamsP::updateItemBackgroundPadding;
        HideColumn = this->handle->GetBool("HideColumn", true);
        funcs["HideColumn"] = &TreeParamsP::updateHideColumn;
        HideScrollBar = this->handle->GetBool("HideScrollBar", true);
        funcs["HideScrollBar"] = &TreeParamsP::updateHideScrollBar;
        HideHeaderView = this->handle->GetBool("HideHeaderView", true);
        funcs["HideHeaderView"] = &TreeParamsP::updateHideHeaderView;
        ResizableColumn = this->handle->GetBool("ResizableColumn", false);
        funcs["ResizableColumn"] = &TreeParamsP::updateResizableColumn;
        ColumnSize1 = this->handle->GetInt("ColumnSize1", 0);
        funcs["ColumnSize1"] = &TreeParamsP::updateColumnSize1;
        ColumnSize2 = this->handle->GetInt("ColumnSize2", 0);
        funcs["ColumnSize2"] = &TreeParamsP::updateColumnSize2;
        TreeToolTipIcon = this->handle->GetBool("TreeToolTipIcon", false);
...
static const App::ParamRegistry::Registrar _TreeParamsRegistrar({
    ...
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "ItemBackground", "ItemBackground", App::ParamInfo::Hex, 0x00000000)
        ...
        .setDoc("Tree view item background. Only effective in overlay.")
        .setOnChange()
        .setProxy("Color")
        .setTransparency(true),
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "ItemBackgroundPadding", "ItemBackgroundPadding", App::ParamInfo::Int, 0)
        .setTitle("Item background padding")
        .setDoc("Tree view item background padding.")
        .setOnChange()
        .setProxy("SpinBox")
        .setRange(0, 100, 1, 0),
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "HideColumn", "HideColumn", App::ParamInfo::Bool, true)
        .setTitle("Hide extra column")
        .setDoc("Hide extra tree view column for item description.")
        .setOnChange(),
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "HideScrollBar", "HideScrollBar", App::ParamInfo::Bool, true)
        .setTitle("Hide scroll bar")
        .setDoc("Hide tree view scroll bar in dock overlay."),
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "HideHeaderView", "HideHeaderView", App::ParamInfo::Bool, true)
        .setTitle("Hide header")
        .setDoc("Hide tree view header view in dock overlay."),
    App::ParamInfo("Gui", "TreeParams", "User parameter:BaseApp/Preferences/TreeView", "ResizableColumn", "ResizableColumn", App::ParamInfo::Bool, false)
        .setTitle("Resizable columns")
