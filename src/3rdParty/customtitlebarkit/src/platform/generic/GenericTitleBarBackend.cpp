...
void GenericTitleBarBackend::detach()
{
    if (m_window) {
        ...
        // LOCAL DIVERGENCE from FreeCAD/FreeCAD#26766, matching the Windows
        // backend: clear the hint attach() set, so switching back to a native
        // title bar at run time actually restores the decoration.
