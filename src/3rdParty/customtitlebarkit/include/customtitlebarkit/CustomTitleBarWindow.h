...
public:
    ...
    /// LOCAL DIVERGENCE from FreeCAD/FreeCAD#26766: switch between a native and
    /// a custom title bar on a window that already exists, so the choice does
    /// not cost a restart. Upstream only takes the mode in the constructor.
    /// Safe to call with the current mode -- it does nothing.
...
Q_SIGNALS:
    ...
    /// LOCAL DIVERGENCE: emitted by setMode() once the swap is complete, so
    /// anything holding widgets that moved (toolbar areas, the menu bar) can
    /// re-attach them.
