...
// [FreeCAD fork addition] See the note on id_base in the header.
uint64_t signal_impl::add_front(packed_function fn)
{
	std::lock_guard lock(m_mutex);

	const uint64_t id = m_nextFrontId - 1;

	m_functions.emplace(m_functions.begin(), std::move(fn));

	try
	{
		m_ids.emplace(m_ids.begin(), id);
	}
	catch (const std::bad_alloc& /*e*/)
	{
		// Remove function since we failed to add its id
