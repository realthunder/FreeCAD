...
// [FreeCAD fork addition] Boost.Signals2 spells this connect(slot, at_front)
// and orders slots by group position. Only the two ends are needed here, and
// at_back is what plain connect() already does, so this is the one tag.
struct at_front_tag
{
};
inline constexpr at_front_tag at_front {};
...
public:
	...
	/**
	 * connect(slot, at_front) method subscribes slot ahead of every slot
	 * connected so far, so it is the first one called on emission.
	 * [FreeCAD fork addition]
	 * @returns connection - object which manages signal-slot connection lifetime
	 */
	connection connect(slot_type slot, at_front_tag)
	{
		const uint64_t id = m_slots->add_front(slot.release());
		return connection(m_slots, id);
	}
	...
	{
		// [FreeCAD fork addition] The local was named `slots`, which Qt
		// #defines to nothing. Any translation unit that reached a Qt header
		// before this one stopped parsing here, so the name cannot be a Qt
		// keyword -- include order is not something a header gets to assume.
		return [weakSlots = detail::signal_impl_weak_ptr(m_slots)](signal_arg_t<Arguments>... args) {
			if (auto locked = weakSlots.lock())
			{
				return locked->invoke<combiner_type, result_type, signature_type, signal_arg_t<Arguments>...>(args...);
			}
		};
	}
