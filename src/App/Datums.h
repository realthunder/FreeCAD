...
/** A local coordinate system: an origin point, three axes and three planes.
 *
 * Upstream derives this publicly from GeoFeatureGroupExtension. This fork
 * holds the extension as a protected member instead, which is what lets
 * App::Origin refuse to save it -- see Origin::canSaveExtension() and
 * Origin::Restore(), and the note on both there.
 */
...
public:
    ...
    /** Creates the datum features if they do not exist yet.
     *
     * Fork-local: upstream creates them eagerly in setupObject(). Here an
     * Origin defers the work until something asks for it, so every getter
     * and the OriginFeatures property itself route through this first.
     */
    ...
    /// Returns true if the given object is one of the datum features
...
protected:
    /** The group extension, held rather than inherited.
     *
     * It answers subname lookups by role prefix ("X_Axis.", "XY_Plane.")
     * and reports the datum features as the export group, neither of
     * which GeoFeatureGroupExtension does on its own.
     */
    ...
    public:
        ...
        bool extensionGetSubObject(DocumentObject*& ret,
                                   ...
                                   bool,
                                   int) const override;
        bool hasObject(const DocumentObject* obj, bool recursive) const override;
    };
