...
/***************************************************************************
 *   Copyright (c) 2015 Stefan Tröger <stefantroeger@gmx.net>  nonascii-ok *
...
void LocalCoordinateSystem::initObjects() const
{
    // Fork-local. Creating six objects and a point costs a recompute and a
    // transaction entry, so it is put off until something actually asks.
    // The guards are the states in which creating a document object is
    ...
    auto* self = const_cast<LocalCoordinateSystem*>(this);
    std::vector<App::DocumentObject*> links;
    for (const auto& data : getSetupData()) {
        auto* feature = self->createDatum(data);
        feature->purgeTouched();
        links.push_back(feature);
    }

    self->OriginFeatures.setValues(links);
}
...
void LocalCoordinateSystem::migrateOriginPoint()
{
    ...
    // Nothing to migrate into: an Origin whose features were never
    // materialized must stay that way, or restoring a document would
    // undo the laziness for every one of them.
    if (features.empty()) {
        return;
    }
...
void LocalCoordinateSystem::LCSExtension::initExtension(ExtensionContainer* cont)
{
    App::GroupExtension::initExtension(cont);
}

const App::PropertyLinkList&
LocalCoordinateSystem::LCSExtension::getExportGroupProperty(int reason) const
{
...
bool LocalCoordinateSystem::LCSExtension::hasObject(const DocumentObject* other,
                                                   [[maybe_unused]] bool recursive) const
{
    return obj && obj->hasObject(other);
}

bool LocalCoordinateSystem::hasObject(const DocumentObject* other) const
{
    // Deliberately does NOT materialize: asking whether an object belongs
    // to an Origin must not be what brings that Origin into existence.
    const auto& features = OriginFeatures.getValues();
    return std::ranges::find(features, other) != features.end();
}
