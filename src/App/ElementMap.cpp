...
/****************************************************************************
 *                                                                          *
 *   Copyright (c) 2024 Zheng, Lei <realthunder.dev@gmail.com>              *
 *                                                                          *
    ...
    void hashChildMaps(ComplexGeoData & master)
    {
        ...
        for (auto & v : this->indexedNames) {
            for (auto & vv : v.second.children) {
                ...
                if (pos > 10) {
                    MappedName postfix = MappedName::fromRawData(child.postfix.constData(), pos)
                        .hashElementName(master.Hasher, child.sids);
                    ss.str("");
                    ss << mappedChildPrefix() << postfix;
                    MappedName tmp;
                          ...
                          const std::vector<MappedChildElements> &children)
    {
            ...
            for (auto iter=indices.children.upper_bound(start); iter!=indices.children.end(); ++iter) {
                ...
                if (grandchild.postfix.size()) {
                    if (entry->postfix.size()
                        && !entry->postfix.startsWith(elementMapPrefix().c_str()))
                    {
                        entry->postfix = grandchild.postfix
                            + elementMapPrefix().c_str() + entry->postfix;
                    } else
        ...
        for (auto & child : expansion.size()?expansion:children) {
            ...
            if (entry->index != 1) {
                ...
                ss.str("");
                ss << elementMapPrefix() << ":C" << entry->index-1;
...
ComplexGeoData::getElementNamesWithPrefix(const char *prefix) const {
    ...
    if(boost::starts_with(prefix,p))
        prefix += p.size();
    names = ->findAllStartsWith(prefix);
    return names;
#else
    (void)prefix;
    return {};
                                       ...
                                       bool forceTag) const
{
    ...
    if (!forceTag && !ss.tellp()) {
        ...
        if(inputTag) {
            ...
            Data::MappedName prev = n.dehashElementName(Hasher);
            long prevTag = 0;
    ...
    if(sids && Hasher) {
        name = name.hashElementName(Hasher, *sids);
                                       ...
                                       std::vector<MappedName> *history) const 
{
    ...
    while(1) {
        ...
        bool dehashed = false;
        if (ret.startsWith(mappedChildPrefix(), len)) {
            int offset = (int)mappedChildPrefix().size();
            MappedName tmp = MappedName::fromRawData(ret, len+offset, pos-len-offset);
            MappedName postfix = tmp.dehashElementName(Hasher);
            if (postfix != tmp) {
                dehashed = true;
        ...
        if (!dehashed) {
            ret = MappedName::fromRawData(ret, 0, len).dehashElementName(Hasher);
        }
...
void ComplexGeoData::traceElement(const MappedName &name, TraceCallback cb) const
{
    ...
    for (int i=0; i<50; ++i) {
        ...
        tmp = tmp.dehashElementName(Hasher);
        names.push_back(tmp);
        encodedTag = 0;
